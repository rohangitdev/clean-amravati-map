...
export default function ReportPage() {
  ...
  const [photo, setPhoto] = useState(null);       // preview data URL
        ...
        <div className="min-h-[80vh] flex items-center justify-center px-4">
          <div className="text-center max-w-sm">
            ...
            <p className="text-slate-500 text-sm mb-6">
              Your garbage report for <strong>{ward}</strong> has been saved to the database.
            </p>
            <div className="flex gap-3 justify-center">
              <button
                onClick={() => {
                  setPhoto(null); setPhotoFile(null); setCoords(null);
                  setGpsStatus('idle'); setNotes(''); setSeverity(3); setSubmitted(false);
                }}
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-4">
            ...
            {gpsStatus === 'success' && coords ? (
              <div className="flex items-center justify-between bg-green-50 border border-green-200 rounded-xl px-3 py-2.5">
                ...
                <button type="button" onClick={handleGPS} className="text-xs text-green-600 underline hover:text-green-800">Retry</button>
              </div>
            ) : (
              <div>
                <button type="button" onClick={handleGPS} disabled={gpsStatus === 'loading'}
                  className={`w-full py-3 rounded-xl text-sm font-semibold flex items-center justify-center gap-2 transition-all ${
                    gpsStatus === 'loading' ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-green-600 text-white hover:bg-green-700 active:scale-[0.98]'
                  }`}>
                  {gpsStatus === 'loading' ? (
                    <><svg className="animate-spin" width="16" height="16" viewBox="0 0 16 16" fill="none">
                      <circle cx="8" cy="8" r="6" stroke="currentColor" strokeWidth="2" strokeDasharray="28" strokeDashoffset="8"/></svg>Getting Location…</>
                  ) : (
                    ...
                    </svg>Capture GPS Location</>
                  )}
                </button>
                {gpsStatus === 'error' && <p className="text-xs text-red-500 mt-2 text-center">{gpsError}</p>}
              </div>
            )}
          </div>
              ...
              {WARDS.map((w) => (
                <button key={w} type="button" onClick={() => setWard(w)}
                  className={`py-2 rounded-xl text-sm font-semibold transition-all ${
                    ward === w ? 'bg-green-600 text-white shadow-sm' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                  }`}>
                  {w.replace('Ward ', 'W')}
                </button>
              ))}
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-4">
              ...
              {[1, 2, 3, 4, 5].map((s) => {
                ...
                return (
                  <button key={s} type="button" onClick={() => setSeverity(s)}
                    style={{ background: isSelected ? info.color : info.bg, color: isSelected ? '#fff' : info.color, border: `2px solid ${isSelected ? info.color : 'transparent'}` }}
            ...
            <div className="mt-2 flex items-center gap-2">
              <span style={{ background: SEVERITY_LABELS[severity].bg, color: SEVERITY_LABELS[severity].color }}
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-4">
            ...
            <textarea value={notes} onChange={(e) => setNotes(e.target.value)}
              placeholder="Describe the garbage situation, landmark, etc." rows={3}
          ...
          <button type="submit" disabled={submitting}
            className={`w-full py-4 rounded-2xl text-base font-bold transition-all flex items-center justify-center gap-2 ${
              submitting ? 'bg-slate-200 text-slate-400 cursor-not-allowed' : 'bg-green-600 text-white hover:bg-green-700 active:scale-[0.98] shadow-lg shadow-green-200'
            }`}>
            {submitting ? (
              <><svg className="animate-spin" width="18" height="18" viewBox="0 0 18 18" fill="none">
                <circle cx="9" cy="9" r="7" stroke="currentColor" strokeWidth="2.5" strokeDasharray="34" strokeDashoffset="10"/>
              </svg>Uploading & Saving…</>
            ) : (
